...
if __name__ == "__main__":
    ...
    model = Colour_Quantisation(num_colours=4)
    transformed_img, probability_map = model(img, training=True)
    prob_mean = torch.mean(probability_map, dim=[2, 3])
    print(prob_mean)
    prob_mean = F.log_softmax(prob_mean)
    print(prob_mean)
    uniform_distribution = torch.tensor([[1/4]]).repeat(2,4)
    print(uniform_distribution)
    kl_div = nn.KLDivLoss(reduction='sum')
    loss = kl_div(prob_mean,uniform_distribution)
    print(loss)
    prob_mean = torch.mean(probability_map, dim=[2, 3])
    Shannon_entropy = -prob_mean * torch.log2(prob_mean+torch.tensor([1e-8]))
