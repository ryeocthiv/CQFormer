...
def main():
    ...
    parser.add_argument('--log_dir', type=str,
                        default='/home/ssh685/CV_project_ICLR2023/Colour-Quantisation-main/logs/CQ_2/tiny200/4_colours/2022-08-29_21-24-41/CQ_best.pth')
    ...
    for i in range(value_size):
        for j in range(hue_size):
            ...
            wcs_pixel = wcs_pixel.unsqueeze(-1).unsqueeze(-1).repeat(1, 64, 64).unsqueeze(0)
            ...
            with torch.no_grad():
                ...
                if max_index == 0:
                    ...
                    colour_3.append(str(j) + '\t' + str(i) + '\n')
                elif max_index == 3:
                    colour_4.append(str(j) + '\t' + str(i) + '\n')
    print(len(colour_1))
    print(len(colour_2))
    print(len(colour_3))
    print(len(colour_4))
    save_txt_root = '/home/ssh685/CV_project_ICLR2023/Colour-Quantisation-main/logs/WCS/{}_colours_new/'.format(
        args.num_colors)
    ...
    with open(os.path.join(save_txt_root, 'colour_3.txt'), 'w') as f:
        f.writelines(colour_3)
    with open(os.path.join(save_txt_root, 'colour_4.txt'), 'w') as f:
        f.writelines(colour_4)


def show_2_colours(dataset):
    # settings
    parser = argparse.ArgumentParser(description='Colour Quantisation')
    parser.add_argument('--seed', type=int, default=0,
                        help='random seed (default: None)')
    args = parser.parse_args()
    log_dir = '/home/ssh685/CV_project_ICLR2023/Colour-Quantisation-main/logs/CQ_3090/tiny200/2_colours/2022-08-30_12-13-54/CQ_best.pth'
    # seed
    if args.seed is not None:
        np.random.seed(args.seed)
        torch.manual_seed(args.seed)
    ...
    # model
    model = Colour_Quantisation(num_colours=2).cuda()
    model.load_state_dict(torch.load(log_dir, map_location='cuda'))
    WCS_r_mat = [
        [245, 246, 247, 248, 249, 249, 249, 247, 254, 253, 247, 241, 233, 227, 223, 217, 212, 210, 208, 208, 207, 207,
         207,
         209, 211, 214, 217, 220, 224, 226, 229, 232, 234, 235, 238, 240, 241, 243, 244, 244, ],
    ...
    for i in range(value_size):
        for j in range(hue_size):
            ...
            with torch.no_grad():
                _, probability_map = model(wcs_pixel, training=False)
                probability_map = probability_map.sum(dim=[2, 3], keepdim=False).squeeze()
                max_index = torch.argmax(probability_map).detach().cpu().numpy()
                if max_index == 0:
                    colour_1.append(str(j) + '\t' + str(i) + '\n')
    ...
    save_txt_root = '/home/ssh685/CV_project_ICLR2023/Colour-Quantisation-main/logs/WCS_1/{}/2_colours/'.format(dataset)
    if not os.path.exists(save_txt_root):
        os.makedirs(save_txt_root)
    with open(os.path.join(save_txt_root, 'colour_1.txt'), 'w') as f:
        f.writelines(colour_1)
...
def show_4_colours(dataset):
    # settings
    parser = argparse.ArgumentParser(description='Colour Quantisation')
    args = parser.parse_args()
    log_dir = '/home/ssh685/CV_project_ICLR2023/Colour-Quantisation-main/logs/CQ_3090/tiny200/4_colours/2022-08-30_12-14-04/CQ_epoch20.pth'
    ...
    # model
    model = Colour_Quantisation(num_colours=4).cuda()
    model.load_state_dict(torch.load(log_dir, map_location='cuda'))
    WCS_r_mat = [
        [245, 246, 247, 248, 249, 249, 249, 247, 254, 253, 247, 241, 233, 227, 223, 217, 212, 210, 208, 208, 207, 207,
         207,
    ...
    print(len(colour_3))
    print(len(colour_4))
    save_txt_root = '/home/ssh685/CV_project_ICLR2023/Colour-Quantisation-main/logs/WCS_1/{}/4_colours/'.format(dataset)
    if not os.path.exists(save_txt_root):
        os.makedirs(save_txt_root)
    with open(os.path.join(save_txt_root, 'colour_1.txt'), 'w') as f:
        f.writelines(colour_1)
...
def show_8_colours(dataset):
    # settings
    parser = argparse.ArgumentParser(description='Colour Quantisation')
    args = parser.parse_args()
    log_dir = '/home/ssh685/CV_project_ICLR2023/Colour-Quantisation-main/logs/CQ_3090/tiny200/8_colours/2022-08-30_12-14-08/CQ_epoch40.pth'
    # seed
    ...
    # model
    model = Colour_Quantisation(num_colours=8).cuda()
    model.load_state_dict(torch.load(log_dir, map_location='cuda'))
    WCS_r_mat = [
        [245, 246, 247, 248, 249, 249, 249, 247, 254, 253, 247, 241, 233, 227, 223, 217, 212, 210, 208, 208, 207, 207,
         207,
        ...
        for j in range(hue_size):
            ...
            with torch.no_grad():
                ...
                if max_index == 0:
                    ...
                    colour_5.append(str(j) + '\t' + str(i) + '\n')
                elif max_index == 5:
                    colour_6.append(str(j) + '\t' + str(i) + '\n')
                elif max_index == 6:
                    colour_7.append(str(j) + '\t' + str(i) + '\n')
                elif max_index == 7:
                    colour_8.append(str(j) + '\t' + str(i) + '\n')
    print(len(colour_1))
    print(len(colour_2))
    print(len(colour_3))
    print(len(colour_4))
    ...
    print(len(colour_7))
    print(len(colour_8))
    save_txt_root = '/home/ssh685/CV_project_ICLR2023/Colour-Quantisation-main/logs/WCS_1/{}/8_colours/'.format(dataset)
    if not os.path.exists(save_txt_root):
        os.makedirs(save_txt_root)
    with open(os.path.join(save_txt_root, 'colour_1.txt'), 'w') as f:
        f.writelines(colour_1)
    ...
    with open(os.path.join(save_txt_root, 'colour_4.txt'), 'w') as f:
        f.writelines(colour_4)
    with open(os.path.join(save_txt_root, 'colour_5.txt'), 'w') as f:
        f.writelines(colour_5)
    with open(os.path.join(save_txt_root, 'colour_6.txt'), 'w') as f:
        f.writelines(colour_6)
    with open(os.path.join(save_txt_root, 'colour_7.txt'), 'w') as f:
        f.writelines(colour_7)
    with open(os.path.join(save_txt_root, 'colour_8.txt'), 'w') as f:
        f.writelines(colour_8)
if __name__ == '__main__':
    dataset = 'tiny200'
    show_8_colours(dataset)
