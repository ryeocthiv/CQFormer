...
class SentinelIndividualImageDataset(SatelliteDataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(selection)
        # print(self.csv_path)
        data_root = os.path.abspath(os.path.join(self.csv_path, ".."))
        # print(data_root)
        if self.is_train:
            image_path = data_root+'/train/{}/{}/{}.tif'.format(str(selection['category']),
                                                                    str(selection['category']) + '_' + str(selection['location_id']),
                                                                    str(selection['category']) + '_' + str(selection['location_id'])+ '_' + str(selection['image_id'])
                                                                    )
        else:
            image_path = data_root+'/val/{}/{}/{}.tif'.format(str(selection['category']),
                                                                str(selection['category']) + '_' + str(selection['location_id']),
                                                                str(selection['category']) + '_' + str(selection['location_id'])+ '_' + str(selection['image_id'])
                                                                )                                                       
        # print(image_path)
        ...
        images = self.open_image(image_path) 
    ...
    def build_transform(is_train, input_size, mean, std):
        ...
        if is_train:
            ...
            t.append(transforms.RandomHorizontalFlip())
            # t.append(transforms.RandomRotation(180))
            return transforms.Compose(t)
...
def build_fmow_dataset(is_train: bool, args) -> SatelliteDataset:
    ...
    if args.dataset_type == 'rgb':
        ...
        dataset = SentinelIndividualImageDataset(csv_path,is_train, transform, masked_bands=args.masked_bands,
...
if __name__ == '__main__':
    ...
    dataset_test = build_fmow_dataset(is_train=False, args=args)
    img_as_tensor, labels = dataset_train.__getitem__(1)
    print(dataset_train.__len__())
    print(torch.max(img_as_tensor))
    print(labels)
